...
function UpdatePost() {
    ...
    useEffect(() => {
        try {
            const fetchPost = async () => {
                const res = await fetch(`https://blog-website-backend-cyan.vercel.app/api/post/getposts?postId=${postId}`)
    ...
    const handleSubmit = async (e) => {
        e.preventDefault()
        try {
            const res = await fetch(`https://blog-website-backend-cyan.vercel.app/api/post/updatepost/${formData._id}/${currentUser._id}`, {
                method: 'PUT',
                headers: {
