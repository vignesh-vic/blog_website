...
function CreatePost() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    try {
      const res = await fetch('https://blog-website-backend-cyan.vercel.app/api/post/create', {
        method: 'POST',
        headers: {
    ...
    <>
      <div className="p-3 max-w-3xl mx-auto min-h-screen">
        ...
        <form className="flex flex-col gap-4 " onSubmit={handleSubmit}>
          ...
          />
          <Button type="submit" gradientDuoTone='purpleToPink' className="mb-4">Publish</Button>
          {
