...
function Comments({ comment, handleLike, commentEdit, handleDelete, token }) {
    ...
    useEffect(() => {
        const getUser = async () => {
            try {
                const res = await fetch(`https://blog-website-backend-cyan.vercel.app/api/user/getby/${comment.userId}`)
                if (res.ok) {
    ...
    const handleSave = async () => {
        try {
            const res = await fetch(`https://blog-website-backend-cyan.vercel.app/api/comment/editComment/${comment._id}`,
                {
                    method: 'PUT',
                    headers: {
